...
import Pyro5.server
import Pyro5.client  # 新增导入
import Pyro5.errors  # 新增导入
import click
...
def _run_pyro_server_process(shutdown_event: multiprocessing.Event, bbox: Tuple[float, float, float, float],
                             workers_count: int, save_cache: bool = False):
    """
    在单独的进程中运行Pyro5服务器。
    """
    ...
    try:
        # 初始化缓存管理器
        CacheManager(save_to_file=save_cache)

        # 初始化进程池执行器，并加载地图数据
        direct_router_executor = DirectRouterProcessExecutor()
        direct_router_executor.init(workers_count)
        futures = [direct_router_executor.submit(load_map_data, bbox) for _ in range(workers_count)]
        for future in futures:
            future.result()

        daemon = Pyro5.server.Daemon(host=SERVER_ADDRESS, port=SERVER_PORT)
        daemon.register(DirectRouter, objectId=PYRO_OBJECT_ID)
        daemon.requestLoop(loopCondition=lambda: not shutdown_event.is_set())
        click.echo(f"DirectRouter RPC服务器 (子进程 PID: {os.getpid()}) 正在关闭...")
        daemon.close()
        CacheManager().shutdown()  # 清理缓存管理器
        DirectRouterProcessExecutor().shutdown()  # 清理进程池执行器
        click.echo(f"DirectRouter RPC服务器 (子进程 PID: {os.getpid()}) 已关闭。")
    except Exception as e:
        click.echo(f"DirectRouter RPC服务器 (子进程 PID: {os.getpid()}) 运行错误: {e}")


class DirectRouterRpcServer:
    """ DirectRouter RPC服务器 """
    ...
    def _wait_for_server_ready(self, timeout: int) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            try:
                ...
                click.echo(f"连接到 DirectRouter RPC 服务器时发生未知错误: {e}")
                return False

        click.echo(f"DirectRouter RPC服务器在 {timeout} 秒内未能成功连接。请检查服务器日志。")
        return False
    ...
    def start_server(self, bbox: Tuple[float, float, float, float], workers: Optional[int] = None,
        ...
        self.server_process = multiprocessing.Process(
            ...
            args=(self.shutdown_event, bbox, workers_count, save_cache)
        )
        ...
        if not self._wait_for_server_ready(connection_timeout):
            ...
            return
        click.echo(f"DirectRouter RPC服务器已成功启动。子进程 PID: {self.server_process.pid}")

    def stop_server(self):
        """ 停止RPC服务器 """
        if not self.is_running() and (self.server_process is None and self.shutdown_event is None):  # 修正判断条件
            click.echo("DirectRouter RPC服务器未运行或已在停止过程中。")
            return

        if self.shutdown_event and not self.shutdown_event.is_set():  # 仅在未设置时设置
            self.shutdown_event.set()  # 通知服务器进程关闭

        if self.server_process:
            click.echo(f"正在等待 DirectRouter RPC 服务器进程 (PID: {self.server_process.pid}) 关闭...")
            self.server_process.join(timeout=10)  # 等待进程结束，最多10秒
            if self.server_process.is_alive():
                click.echo(f"RPC 服务器进程 (PID: {self.server_process.pid}) 未能优雅关闭，将尝试强制终止。")
                self.server_process.terminate()  # 如果超时仍未结束，则强制终止
                self.server_process.join(timeout=5)  # 等待终止完成
                if self.server_process.is_alive():
                    click.echo(f"RPC 服务器进程 (PID: {self.server_process.pid}) 强制终止失败。")
                else:
                    click.echo(f"RPC 服务器进程 (PID: {self.server_process.pid}) 已被强制终止。")
            else:
                click.echo(f"DirectRouter RPC 服务器进程 (PID: {self.server_process.pid}) 已成功关闭。")

        self.server_process = None
        self.shutdown_event = None  # 确保在停止后重置

        click.echo("DirectRouter RPC服务器停止流程已执行。")
